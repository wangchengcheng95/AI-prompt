...
- **gstack** — vendored under `platforms/gstack/` (upstream MIT). Cursor-style agents discover skills via repo-root `.agents/skills/` (symlinks into `platforms/gstack`). After clone, run `bash scripts/bootstrap-gstack.sh` once to build `browse/` locally (Bun + Playwright; build output is gitignored). Reference-only checkout remains at `references/external/gstack`.
