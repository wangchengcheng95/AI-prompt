---
name: prompt-sync
description: Preview and run manifest-driven prompt asset sync and backflow for this repository. Use when Codex needs to copy archived platform assets from this repo into a local sibling workspace or repository, check drift for a managed target, or backflow a user-specified managed file or directory from a local target back into this upstream repository.
---
...
- Treat root `.codex/` content as repo-local by default; do not export it unless the manifest explicitly says so.
