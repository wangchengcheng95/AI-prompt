---
name: pr-operator
description: Manage repo-local GitHub pull request operations with a bundled wrapper script. Use when Claude needs to push a task branch for PR work, create a pull request, view an existing pull request, or update a pull request title or body.
---
...
- From the repository root, invoke it as `./.claude/skills/pr-operator/scripts/prctl`.
...
- Use other standard types such as `refactor`, `test`, `build`, `ci`, `perf`, `style`, or `chore` only when they better match the actual diff or PR purpose.
...
```bash
PRCTL="./.claude/skills/pr-operator/scripts/prctl"
