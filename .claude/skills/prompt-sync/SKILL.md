---
name: prompt-sync
description: Preview and run manifest-driven prompt asset sync and backflow for this repository. Use when Claude needs to copy archived platform assets from this repo into a local sibling workspace or repository, check drift for a managed target, or backflow a user-specified managed file or directory from a local target back into this upstream repository.
---
...
The skill keeps operator guidance under the Claude skill directory, while the sync system itself remains repo-local infrastructure.
...
- Treat root `.claude/` and root `.codex/` repo-maintenance assets as local by default; do not export them unless the manifest explicitly says so.
